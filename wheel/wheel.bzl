...
def _generate_manifest(ctx, package_name):
    ...
    ctx.actions.expand_template(
        ...
        output=manifest,
        substitutions={"{manifest}": manifest_text},
        is_executable=True,
    )
...
def _bdist_wheel_impl(ctx):
    ...
    setup_py_dest_dir = "/".join(
        [package_dir.path, "/".join(build_file_dir.split("/")[:-1]), ctx.attr.strip_src_prefix.strip("/")]
    )
    ...
    # Create working directory in same steps as generation re sandboxing
    # https://groups.google.com/d/msg/bazel-discuss/UCWmUdjJ11o/TVnfiqPFAwAJ
    command = (
        "mkdir -p {package_dir} "
        + "&& chmod 0775 {package_dir} "
    ...
    ctx.actions.run_shell(
        mnemonic="BuildWheel",
        outputs=[package_dir, ctx.outputs.wheel],
...
_bdist_wheel_attrs = {
    "srcs": attr.label_list(
        doc="Source files to include in the wheel", allow_files=[".py"], mandatory=True, allow_empty=False
    ),
    ...
    "version": attr.string(default="0.0.1", doc="Version to be assigned to the wheel.", mandatory=False),
    "description": attr.string(doc="Short description of the wheel, no more than 200 characters.", mandatory=False),
    "classifiers": attr.string_list(doc="Classifiers for the wheel.", mandatory=False),
    "platform": attr.string_list(default=["any"], doc="Platform the wheel is being built for.", mandatory=False),
    ...
    "install_requires": attr.string_list(
        doc="A list of strings specifying what other wheels need to be installed when this one is.", mandatory=False
    ),
    "_setup_py_template": attr.label(default=Label("//wheel:setup.py.template"), allow_single_file=True),
    "_manifest_template": attr.label(default=Label("//wheel:MANIFEST.in.template"), allow_single_file=True),
}
...
bdist_wheel = rule(
    implementation=_bdist_wheel_impl, executable=False, attrs=_bdist_wheel_attrs, outputs=_bdist_wheel_outputs
)
